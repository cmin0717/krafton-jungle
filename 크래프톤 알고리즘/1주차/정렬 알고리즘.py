...
def quick(arr):
    ...
    for i in arr:
        if i > arr[pivot]:
            ...
            mid_arr.append(i)
    return quick(start_arr) + mid_arr + quick(end_arr) # quick(mid_arr)을 안하는 이유는 mid_arr에 같은 값이 들어가면 무한 루프에 빠진다.
print(quick(array))
...
arr = [4, 3, 5, 1, 2]
def heap_sort(arr):
    heapify(arr) # 입력받은 리스트를 heap으로 변환
    sort_num = []

    while arr:
        sort_num.append(heappop(arr)) # 최소값을 하나씩 sort_num리스트에 넣어준다.
